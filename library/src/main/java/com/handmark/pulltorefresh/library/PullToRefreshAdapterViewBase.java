...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public final void onScrollStateChanged(final AbsListView view, final int state) {
		 ...
		 * visible.
		 */
		// 三个条件同时为真时，调用onLastItemVisible()
		// OnScrollListener.SCROLL_STATE_IDLE：滑动状态为停止
		// null != mOnLastItemVisibleListener：该接口的引用不为null
		// mLastItemVisible最后一个item是否可见
	...
	private boolean isFirstItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			 ...
			 * getFirstVisiblePosition()：获取第一个可见的item的position
			 * getChildAt(int index)：获取第index个可见的view,这个index必须属于可见的position
			 * firstVisibleChild.getTop()：
			 * mRefreshableView.getTop()：listView到其父view左上角的垂直高度
			 * firstVisibleChild.getTop()：第一个可见的view到listView左上角的垂直高度
			 * 这里获取ListView的第一个条目，然后判断该条目是否完全可见
			 * @return
			 * 该框架中的listView有一个header，但是该header的高度为0，visible(可见)
			 */
			if (mRefreshableView.getFirstVisiblePosition() <= 1) {  //
				final View firstVisibleChild = mRefreshableView.getChildAt(0);
				Log.e("getHeight", firstVisibleChild.getHeight()+"");
				Log.e("getVisibility", firstVisibleChild.getVisibility()+"");
				Log.e("View.GONE", View.GONE+"");
				Log.e("View.Visible", View.VISIBLE+"");
				Log.e("View.INVisible", View.INVISIBLE+"");
				Log.e("firstVisiblePosition", mRefreshableView.getFirstVisiblePosition()+"");
				Log.e("firstVisibleChild", firstVisibleChild.getTop()+"");
				Log.e("listView", mRefreshableView.getTop()+"");
				if (firstVisibleChild != null) {
					return firstVisibleChild.getTop() >= 0;
					// mRefreshableView代表ListView
					// ListView由FrameLayout包裹，所以listView的getTop()方法返回的永远是0
					// 该框架中firVisibleChild
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			// mRefreshableView.getCount() - 1：获取ListView的最后一个item的position
			final int lastItemPosition = mRefreshableView.getCount() - 1;
			// mRefreshableView.getLastVisiblePosition()：获取ListView最后一个可见item的position
